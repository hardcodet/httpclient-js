...
export class HttpClient {
    ...
    _invoke(method: any, uri: string, data: any, headers: any): Promise<ApiResponse> {
        return this._invokeWithRetries(method, uri, data, headers, 1);
    }


    async _invokeWithRetries(method: any, uri: string, data: any, headers: any, attemptCounter: number): Promise<ApiResponse> {
        try {
            ...
            if (success || attemptCounter >= this.options.maxAttempts || res.status < 500 && res.status !== 401) {
                return new ApiResponse(res, undefined, attemptCounter);
            }

            if (res.status === 401 && attemptCounter === 1 && this.options.authClient) {
            ...
            return await this._invokeWithRetries(method, uri, data, headers, attemptCounter + 1);
        } catch (error) {
    ...
    private async parseResultInternal<T>(response: ApiResponse): Promise<ApiResult<T>> {
        return HttpClient.parseResult<T>(response, this.inboundProcessors);
    }


    static async parseResult<T>(response: ApiResponse, inboundProcessors?: IJsonProcessor[]): Promise<ApiResult<T>> {
        try {
            if (!response.success) {
                // just wrap the original result
                return new ApiResult<T>(undefined, response.response, response.error, response.attempts);
            }
            ...
            return new ApiResult<T>(obj, response.response, undefined, response.attempts);
        } catch (e) {
            return new ApiResult<T>(undefined, response.response, e, response.attempts);
        }
    }
